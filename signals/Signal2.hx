...
class Signal2<T1, T2>{
  var l:Array<T1->T2->Void>; // listeners
  var o:Array<T1->T2->Void>; // once listeners
  ...
  public function add(f:T1->T2->Void, ?prioritize:Bool = false){
    if(prioritize)
      l.insert(0, f);
    else
      l[listeners] = f;
  ...
  public function once(f:T1->T2->Void, ?prioritize:Bool = false){
    if(prioritize)
      o.insert(0, f);
    else
      o[listenersOnce] = f;
    ...
    inline
  #end
  public function dispatch(a1:T1, a2:T2){
  ...
  public function remove(f:T1->T2->Void){ // removes first occurence in l
    var i = listeners;
