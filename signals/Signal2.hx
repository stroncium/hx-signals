package signals;
class Signal2<T1, T2>{
  var l:Array<T1->T2->Void>; // catchers
  var o:Array<T1->T2->Void>; // "once" catchers
  ...
  public function add(f:T1->T2->Void){
    l[ll] = f;
    ll++;
    }

  public function once(f:T1->T2->Void){
    o[ol] = f;
    ol++;
    }
  ...
  public function remove(f:T1->T2->Void){ // removes first accourence in l
    var i = 0;
    while(i < ll){
      if(l[i] == f)
        break;
