package signals;
class Signal1<T1>{
  var l:Array<T1->Void>; // catchers
  var o:Array<T1->Void>; // "once" catchers
  ...
  public function add(f:T1->Void){
    l[ll] = f;
    ll++;
    }

  public function once(f:T1->Void){
    o[ol] = f;
    ol++;
    }
  ...
  public function remove(f:T1->Void){ // removes first accourence in l
    var i = 0;
    while(i < ll){
      if(l[i] == f)
        break;
